    ...
    public class BlobStorageTests
    {
        ...
        public async void AddChunk()
        {
            ...
            using (var s = new BlobStorage("test.blob"))
            {
                ...
                var res = await s.ReadChunks(c => c.Id == 1);
        ...
        public async void ChunkIdReuse()
        {
            ...
            using (var s = new BlobStorage("test.blob"))
            {
                ...
                await s.RemoveChunk(sc => sc.Chunks.FirstOrDefault(c => c.Id == c1.Id));
                await s.RemoveChunk(sc => sc.Chunks.FirstOrDefault(c => c.Id == c2.Id));
        ...
        public async void RemoveChunk()
        {
            using (var s = new BlobStorage("test.blob"))
            {
                ...
                var res = await s.ReadChunks(c => c.Id == c1.Id);
                Assert.Equal(data, res.First().Data);
        ...
        public async void SpaceReuse()
        {
            using (var s = new BlobStorage("test.blob"))
            {
                ...
                res = await s.ReadChunks(c => c.Id == c2.Id);
                Assert.Equal(data, res.First().Data);
                ...
                res = await s.ReadChunks(c => c.Id == c3.Id);
                Assert.Equal(data, res.First().Data);
